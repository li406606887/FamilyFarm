...
//    [_player play];
    [self useDelegateWith:LZAVPlayerStatusLoadingVideo];
}
...
/**
 ...
 @param status 播放状态
 */
- (void)useDelegateWith:(LZAVPlayerStatus)status
{
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSKeyValueChangeKey,id> *)change context:(void *)context
{
    ...
    if ([keyPath isEqualToString:@"status"]) {// 播放状态
        ...
        if (self.isCanPlay) {
            NSLog(@"跳转后没数据");
            self.needBuffer = YES;
            [self useDelegateWith:LZAVPlayerStatusCacheData];
        }
        ...
        if (self.isCanPlay && self.needBuffer) {
            ...
            [self useDelegateWith:LZAVPlayerStatusCacheEnd];
        }
...
- (void)handleStatusWithPlayerItem:(AVPlayerItem *)item
{
    ...
    switch (status) {
        case AVPlayerItemStatusReadyToPlay:   // 准备好播放
            ...
            self.isCanPlay = YES;
            [self useDelegateWith:LZAVPlayerStatusReadyToPlay];
            
            break;
        case AVPlayerItemStatusFailed:        // 播放出错
            
            NSLog(@"AVPlayerItemStatusFailed");
            [self useDelegateWith:LZAVPlayerStatusItemFailed];
...
{
    NSLog(@"视频播放结束");
    [self useDelegateWith:LZAVPlayerStatusPlayEnd];
...
{
    NSLog(@"视频异常中断");
    [self useDelegateWith:LZAVPlayerStatusPlayStop];
}
/** 进入后台 */
- (void)videoPlayEnterBack:(NSNotification *)notic
{
    NSLog(@"进入后台");
    [self useDelegateWith:LZAVPlayerStatusEnterBack];
}
/** 返回前台 */
- (void)videoPlayBecomeActive:(NSNotification *)notic
{
    NSLog(@"返回前台");
    [self useDelegateWith:LZAVPlayerStatusBecomeActive];
}
